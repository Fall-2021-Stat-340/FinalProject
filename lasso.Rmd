...
```{r}
head(ind5, 5)
# eliminate variants of GDP
ind5_clean <- ind5[,-c(51,52,61,66,67,68,71,72,73,74,80,81,82)]
dim(ind5_clean)
```
```{r}
library(MASS);
lambdas = c(1,2,5,10,20,30,50,100,200,500)
ridge_models<-lm.ridge(`GDP_per_capita_(current_US$)`~.,ind5_clean,lambda = lambdas)
plot(ridge_models)
```
```{r}
coef(ridge_models)
...
```{r}
ind5_predictors <- cbind(const=1,ind5_clean[,-c(59)])
...
```
```{r}
obs_by_lambda-predictions <- as.matrix()
```
